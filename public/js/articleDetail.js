$(document).ready(function(){
	...
	*/
	$(".caracteristique #submit").on("click",function(){
		var newTypicalName=$("#typicalName").val() ? $("#typicalName").val() : "";
		var newTypicalValue=$("#typicalValue").val() ? $("#typicalValue").val() :"";
		var articleId=$("#articleId").text();
		var articleClasse=$("#articleClasse").text();
		var articleCategory=$("#articleCategory").text();
		
		console.log(articleId);
		console.log(articleClasse);
		console.log(articleCategory);
		
		if(newTypicalName.length>0 && newTypicalValue.length>0){
			$.ajax({
	...
	function editarticletypical(){
		$(".caracteristique .edit").on("click",function(){
			$("input:first-child").removeClass("required");
			$("input.typicalvaluespan").removeClass("required");
			var id=($(this).parent("li")).attr("id");
			console.log(id);
			var newName=(($(this).parent()).find("input:first-child")).val();
			var newValue=(($(this).parent()).find("input.typicalvaluespan")).val();
			if(id.length>0){
				if(newName.length>0 && newValue.length>0){
					$.ajax({
						url:"/admin/editarticletypical/"+id,
						dataType:"json",
						type:"POST",
						data:"typicalName="+newName+"&typicalValue="+newValue,
						error:function(err){
							console.log("err");
							console.log(err);
						},
						success:function(response){
							...
							if(response.noTypical){
								console.log("noti")
							}
							if(response.notypicalName){
								console.log("noname");
							}
							if(response.notypicalValue){
								console.log("novalue");
							}
						}
					});
					...
					if(!newName){
						($(this).parent()).find("input:first-child").addClass("required");
					}
					if(!newValue){
						($(this).parent()).find(".typicalvaluespan").addClass("required");
					}
				}
			}
		});
	...
	editarticletypical();
	function removearticletypical(){
		$(".caracteristique .remove").on("click",function(){		
			var id=($(this).parent("li")).attr("id");
			var elem=$(this).parent("li");
			if(id.length>0){
				$.ajax({
						url:"/admin/removearticletypical/"+id,
						dataType:"json",
						data:"",
						type:"POST",
						error:function(err){
